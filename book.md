...
[F2: 시스템] - 4: 응용파라미터 - 24: 힘제어 - 3: 힘제어 옵션
...
| **사용자 좌표계(UCS ID)**     | 사용자 정의 좌표계 번호 - Crd가 User일 경우 사용 |
...
- 툴 좌표계 번호(ID)는 "[F2: 시스템] - 4: 응용파라미터 - 24: 힘제어 - 2: 힘제어 툴 데이터" 에서 설정된 툴 번호를 의미 합니다
...
- **이름 (Name)**: cnd_12  
- **설명 (Description)**: Sanding 작업 조건  
- **좌표계 (Crd)**: Tool 좌표계 선택  
- **툴 좌표계 (Tool ID)**: 0번 힘 제어 툴 정보 사용 (질량 및 무게중심 적용)  
- **영점 기능 (Zeros)**: ON → 힘 제어 시작 시 센서 값을 0으로 초기화하여 사용  
...
축별 세부 제어는 실제 작업 조건(예: 표면 곡률, 정밀도 요구도 등)에 따라 조정해야 하며,  Gain이 낮을수록 더 유연한 반응을 얻을 수 있습니다.


강성 비율을 낮으면 유연한 반응을 얻지만, 로봇 응답과 환경에 따라 진동과 소음을 유발 할 수 있습니다.
...
- 설정 기준: 3초 동안 아래 기준으로 접촉을 유지하면, 접촉면 판단 OK
