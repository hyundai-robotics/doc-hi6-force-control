...
![OnRobot HEX 설정](../_assets/_02_fctrl_env_setting_OnRobot_HEX_E_UDP.png)
...
![AIDIN 센서 설정](../_assets/_21_fctrl_env_set_AIDIN.png)
...
{% endhint %}### 4.1 힘 제어 옵션 - 기본 (Settings)
...
{% endhint %}### 4.4.1 힘 제어 옵션 - 프로파일 : 소프트 스타트
...
{% endhint %}### 4.5.1 힘 제어 옵션 - 모션 : 접촉 조건
...
delay 5                             # 외력 안정화 대기
fctrl off                           # 힘 제어 종료## 5.2 예제: 제어 설정 변경 
...
| 항목명 | 설명 |
| :--- | :--- |
| **F** | 실시간 목표 힘 입니다. [단위: $\text{N}$ 또는 $\text{Nm}$] |
| **Cmd** | 힘 제어를 추종하기 위해 시스템에서 실시간으로 생성하는 명령 위치입니다. [단위: $\text{mm}$ or $\text{deg}$] |
| **Vmax** | 시스템 안정성을 위해 설정된 최대 제한 속도 프로파일 데이터입니다. [단위: $\text{mm/s}$] |
